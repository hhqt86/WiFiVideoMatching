...
public class Suntec_Level1_BigScreen {
    ...
    static int[] fpPointInTheArea = new int[41];
	...
	static String fileWifiInput = "output_Suntec_12_28_30_to_12_33_00.csv";
	static String fileOutputFolder = "src/SuntecRealData/BigScreenExp/output/";
	...
	static void initializeTheArray() {
		for (int i = 0; i <= 40; i++) {
			fpPointInTheArea[i] = i + 10;
		}
		startingHour = 12; startingMinute = 28; startingSecond = 30;
		endingHour = 12; endingMinute = 38; endingSecond = 00;
...
/*		int saveX = -10;
	...
	public static boolean wifiAppearInTheExamineArea(int[] wifi) {
		for (int i = 0; i < experimentNumber; i++) {
			if ((wifi[i]+"").charAt(0)=='1'){
				for (int k = 0; k < fpPointInTheArea.length; k++) {
					if ((wifi[i] % 1000) == fpPointInTheArea[k]) {
						return true;
					}
				}
			}
	...
	public static void readWifiData() {
	    ...
	    try {
	        ...
	        while (scan.hasNextLine()) {
	        	...
	        	if (timeStep >= 0) {
	        		int locationLevel = parser.getLocationLevel(); 
		        	peopleWifiTemp[ID][timeStep] = locationLevel * 1000 + parser.getLocation();
		...
		try {				
			...
			for (int i = 0; i < blobWifiNumber; i++) {
				writer.write(i + " " + indexOfTheRealWifi[i] + "\n");
			}
			writer.close();
