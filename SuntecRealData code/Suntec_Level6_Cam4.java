...
public class Suntec_Level6_Cam4 {
	...
	static void initializeTheArray() {
		...
		peopleVideo = new timeWithLocationCoordinate[Config.maximumBlobCount][experimentNumber];		
		peopleWifi = new timeWithLocation[Config.maximumBlobCount][Config.maximumWifiReportCount];
		isGoodSample = new boolean[Config.maximumBlobCount];
		countTimeWifiReport = new int[Config.maximumBlobCount * 100];
		countTimeWifiReportAfterCleaning = new int[Config.maximumBlobCount * 100];
	...
	public static int getNearestLandMarkInt(int level, Coordinate location) {
		...
		for (int i = 0; i < map[level].fpPointInThisMap; i++) {
			if (map[level].fpCoordinate[i].x != Config.noPoint.x && map[level].fpCoordinate[i].y != Config.noPoint.y) {
	...
	public static void convertToPhysicalCoordinate(int level) {
		//(1920 x 1080)	
		for (int blobID = 0; blobID < blobVideoNumber; blobID++) {
			for (int frame = blobDuration[blobID].left; frame <= blobDuration[blobID].right; frame++) {
				if (blobImageCoor[blobID][frame] != null) {
					int timeStep =getTimeStampVideo(frame); //video is 25 fps
					...
					if (blobPhysicalCoor[blobID][timeStep] == null) {
						blobPhysicalCoor[blobID][timeStep] = convertToPhysicalCoor(level,footLocation);
						//peopleVideo[blobID][timeStep] = getNearestLandMarkInt(level,blobPhysicalCoor[blobID][timeStep]);
						peopleVideo[blobID][timeStep] = new timeWithLocationCoordinate(level, blobPhysicalCoor[blobID][timeStep], timeStep);
					}
				}
			}
		}				
	...
	public static void readWifiData() {
	    ...
	    ParserWifiRTLS parser;
	    int ID = 0;
	    countTimeWifiReport = new int[Config.maximumBlobCount];
	    try {
    ...
    public static timeWithLocationCoordinate[] fillInterpolateData(timeWithLocationCoordinate timeLocation1, timeWithLocationCoordinate timeLocation2) {
    	if (!(timeLocation1.location.level == 1 && timeLocation2.location.level == 4) || (timeLocation1.location.level == 4 && timeLocation2.location.level == 1)) { 
	    	...
	    	for (int i = 1; i < duration; i++) {
	    		...
	    		result[i] = new timeWithLocationCoordinate(currentLevel, currentLocation, i);
	    	}
	    	return result;
    	}
    	else {
	    ...
	    try {
	    	...
	    	for (int ID = 0; ID < numberOfWifi; ID++) {
	    		for (int k = 0; k < countTimeWifiReportAfterCleaning[ID]; k++) {	    	
	    			writer.write(ID + "," + peopleWifiAfterCleaning[ID][k].time + "," + peopleWifiAfterCleaning[ID][k].location.level + "," + peopleWifiAfterCleaning[ID][k].location.locationCoor.x + "," + peopleWifiAfterCleaning[ID][k].location.locationCoor.y);
	    			writer.write("\n");
	    		}
	    	}	  
	        writer.close();
    ...
    public static void readAfterInferLocationDueToBouncingSignal() {
	    ...
	    int ID = 0;	 
	    numberOfWifi = 0;
	    peopleWifiAfterCleaning = new timeWithLocationCoordinate[Config.maximumBlobCount][5000];
	    try {
    ...
    public static Coordinate findWeightMeanCluster(int ID, int left, int right) {
    	...
    	for (int i = 0; i < countInList; i++) {
    		if (map[locationList[i].level].fpCoordinate[locationList[i].locationID].x != Config.noPoint.x &&
    				map[locationList[i].level].fpCoordinate[locationList[i].locationID].y != Config.noPoint.y) { //the location is far from observed area
    			sumWeight += weight[i];
    ...
    public static void normalizeTheBouncingLocation(int ID, int[] clusterID) {
    	...
    	for (int i = 1; i <= countTimeWifiReport[ID]; i++) {
    		if (clusterID[i] != clusterID[i - 1]) {
    			...
    			//smooth from (t0 + t1)/2
    			if (startCluster == 0 && map[level].fpCoordinate[peopleWifi[ID][0].location.locationID].x != Config.noPoint.x &&
    					map[level].fpCoordinate[peopleWifi[ID][0].location.locationID].y != Config.noPoint.y) {//For the first cluster
	    ...
	    if (map[level].fpCoordinate[peopleWifi[ID][countTimeWifiReport[ID] - 1].location.locationID].x != Config.noPoint.x &&
	    		map[level].fpCoordinate[peopleWifi[ID][countTimeWifiReport[ID] - 1].location.locationID].y != Config.noPoint.y) {	    	
    ...
    public static void interpolate() {
    	peopleWifiAfterInterpolate = new timeWithLocationCoordinate[Config.maximumBlobCount][Config.maximumWifiReportCount];
    	countTimeWifiReportAfterInterpolate = new int[Config.maximumWifiReportCount];
    	for (int ID = 0; ID < numberOfWifi; ID++) {
    		System.out.println(ID);
    		if (ID == 15) {
	...
	public static void createMap() {
		...
		for (int i = 0; i < map[1].fpPointInThisMap; i++) {
			map[1].fpCoordinate[i] = new Coordinate(Config.noPoint.x, Config.noPoint.y);
		}
		map[1].numberOfAP = 13;
		...
		for (int i = 0; i < map[6].fpPointInThisMap; i++) {
			map[6].fpCoordinate[i] = new Coordinate(Config.noPoint.x, Config.noPoint.y);
		}
		map[6].numberOfAP = 30;
		map[6].apLocationAtFP = new APLocation[] {new APLocation("156.28.18.192.33.238", 6, 151),
				new APLocation("156.28.18.192.54.136", 6, 52),
				new APLocation("156.28.18.192.33.230", 6, 46),
				new APLocation("156.28.18.192.53.240", 6, 140),
				new APLocation("156.28.18.192.33.216", 6, 42),
				new APLocation("156.28.18.192.54.254", 6, 39),
		...
		map[6].fpCoordinate[129] = new Coordinate(14,0);
		...
		map[6].fpCoordinate[153] = new Coordinate(0,0);
		map[6].fpCoordinate[154] = new Coordinate(7.5,-1);
		...
		for (int i = 0; i < map[6].numberOfPhysicalVideoX; i++) {
			for (int j = 0; j < map[6].numberOfPhysicalVideoY; j++) {
				if (map[6].videoCoordinate[i][j] != null) {
					map[6].videoCoordinate[i][j].x = (map[6].videoCoordinate[i][j].x / map[6].videoDimensionInCM.x) * map[6].videoResolution.x;
					map[6].videoCoordinate[i][j].y = (map[6].videoCoordinate[i][j].y / map[6].videoDimensionInCM.y) * map[6].videoResolution.y;
				}
			}
		}
		//For level 4
		...
		for (int i = 0; i < map[4].fpPointInThisMap; i++) {
			map[4].fpCoordinate[i] = new Coordinate(Config.noPoint.x, Config.noPoint.y);
		}
		map[4].apLocationAtFP = new APLocation[] {new APLocation("156.28.18.192.33.46", 4, 544),
				...
				new APLocation("108.243.127.198.206.72", 4, 620),
				new APLocation("108.243.127.198.201.36", 4, 602),
				new APLocation("108.243.127.198.205.206", 4, 599),
				new APLocation("108.243.127.198.205.68", 4, 614),
				new APLocation("108.243.127.198.209.4", 4, 617),
		...
		map[4].fpCoordinate[591] = new Coordinate(-6.5,0);
		map[4].fpCoordinate[595] = new Coordinate(-3,0);
		map[4].fpCoordinate[597] = new Coordinate(-1,0);
		map[4].fpCoordinate[598] = new Coordinate(1,-1);
		...
		map[4].fpCoordinate[612] = new Coordinate(4.5,3.5);
		...
		map[4].fpCoordinate[618] = new Coordinate(9.5,0);
		...
		for (int i = 0; i < map[4].numberOfPhysicalVideoX; i++) {
			for (int j = 0; j < map[4].numberOfPhysicalVideoY; j++) {
				if (map[4].videoCoordinate[i][j] != null) {
					map[4].videoCoordinate[i][j].x = (map[4].videoCoordinate[i][j].x / map[4].videoDimensionInCM.x) * map[4].videoResolution.x;
					map[4].videoCoordinate[i][j].y = (map[4].videoCoordinate[i][j].y / map[4].videoDimensionInCM.y) * map[4].videoResolution.y;
				}
			}
		}	
