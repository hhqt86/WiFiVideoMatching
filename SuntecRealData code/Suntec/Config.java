...
public class Config {
	...
	static int maximumBlobVideoCount = 10000;  // Maximum blob count in the Suntec video
	static int maximumFrameCount = 100000;// Maximum frame count in the Suntec video, 30 fps
	static int maximumWifiReportCount = 7500;
	static double averageScoreOfLeaderThreshold = 0.7;
	static double rtRealWifi = 0.25;
	static NormalLocationCoordinate noPoint = new NormalLocationCoordinate(new Coordinate(-1000,-1000));
...
class NormalLocation{
	int locationID;	
	public NormalLocation(int location) {
	...
	public NormalLocation(String location) {	
		this.locationID = Integer.parseInt(location.substring(location.length() - 2));
	}
...
class NormalLocationCoordinate {
	...
	public NormalLocationCoordinate(Coordinate coor) {
...
class APLocation {
	String macAddress;
	Coordinate location;
	public APLocation(String macAddress, double coordinateX, double coordinateY) {
		this.macAddress = macAddress;
		this.location = new Coordinate(coordinateX, coordinateY);
	}
...
class TimeWithLocation {
	...
	public TimeWithLocation(int location, int time) {
		this.time = time;
		this.location = new NormalLocation(location);
	}
...
class RankTimeWithLocationCoordinate {
	...
	public RankTimeWithLocationCoordinate(int rank, int time, double coorX, double coorY) {
		this.rank = rank;
		this.time = time;
		this.location = new NormalLocationCoordinate(new Coordinate(coorX, coorY));
	}
...
class timeWithLocationCoordinate {
	NormalLocationCoordinate location;
	int time;
	public timeWithLocationCoordinate(Coordinate coor, int time) {
		this.time = time;
		this.location = new NormalLocationCoordinate(coor);
	}
